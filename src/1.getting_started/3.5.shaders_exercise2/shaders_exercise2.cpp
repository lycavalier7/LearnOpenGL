...
int main()
{
	...
	while (glfwWindowShouldClose(pWindow) == false)
	{
		...
		uint32_t locationColor = glGetUniformLocation(shaderProgram, "outColor");
		glUniform4f(locationColor, 0.0, greenValue, 0.0, 1.0);
		uint32_t locationBias = glGetUniformLocation(shaderProgram, "bias");
		glUniform1f(locationBias, 0.5);
		glBindVertexArray(VAO);

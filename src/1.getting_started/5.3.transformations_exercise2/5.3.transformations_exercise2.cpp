...
int main()
{
	...
	Shader shader("5.3.transformations.vs", "5.3.transformations.fs");
	...
	while (glfwWindowShouldClose(pWindow) == false)
	{
		...
		uint32_t matrixLocation = glGetUniformLocation(shader.programId, "transform");
		glUniformMatrix4fv(matrixLocation, 1, false, glm::value_ptr(transform));
		...
		glm::vec3 scaleFactorVec = glm::vec3(scaleFactor);
		transform = glm::scale(transform, scaleFactorVec);
